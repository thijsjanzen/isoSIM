...
FST
```
Furthermore, with increasing overlap, we observe a decrease in FST:
```{r}
found <- c()
for(overl in seq(0.0, 1.0, by = 0.1)) {
  two_populations <- create_two_populations(pop_size = 1000, 
                                          number_of_founders = 10,
                                          total_runtime = 100, 
                                          morgan = 1, 
  ...
  found <- rbind(found, c(overl, FST))
  cat(overl, FST, "\n")
}

plot(found[,2]~found[,1], xlab = "Overlap", ylab = "FST")
```
